...
case class mediumStrategy() extends de.htwg.se.model.fieldComponent.fieldBaseImpl.difficulties.Strategy {
    ...
    override def put(field: FieldInterface) = {limitV = field.size; limitH = field.size2; if(evalNext(field)) plannedNext else evalNew(field)}
    ...
    def evalNext(field: FieldInterface):Boolean = { //true if there is a better reaction (prioritizing)
        ...
        val myDataSet = WinCheck.checkWinWithNumber(field, ComputerStone) //limit Scanning on my own Stones
        try {        
            if (myDataSet._2 == 3) { // 3er Row found
                for (x <- myDataSet._3) { // jede Row
                    var possiblePos = (0,0)
                    if (x._1 == 3) {  // Wenn Set aus 3*StonePos besteht
                        val freePossibilities = relevantPos(field, predict(x._2)) //übergebe Set; bekomme freie Möglichkeiten
                        for (i <- 1 until freePossibilities.size) { // skip ZombieElement
                            ...
                            if (matchedPos == possiblePos._1) {plannedNext = possiblePos; throw FoundWin}
                        }
                    }
                }
                plannedNext = (0, 0)
    ...
    def relevantPos(field: FieldInterface, predictList: List[(Int, Int)]) : List[(Int, Int)] = { //removes Positions that are already filled (DO NOT INCLUDE matchX and check only if Stone.Empty)
    ...
    def predict(StoneSet : List[(Int, Int)]) : List[(Int, Int)] = {
        ...
        if (xDiff == 0) { //horizontal (2 cases) da x invariabel
            ...
            var mL = List((invariabel, StoneSet(0)._2 -1)) // -1 returns no EmptyStone if asked via field.get (case 1 L)
            ...
            var mL = List((StoneSet(0)._2 -1, invariabel)) // -1 returns no EmptyStone if asked via field.get (case 1 O)
            ...
            if (xDiff == 1 & yDiff == 1) { //R+U = L+O (x-1; y-1 // x+1; y+1) => 1;1
                returnDiagonal(1, 1, StoneSet(StoneSet.size-1), StoneSet(0)) // wir gehen in den ersten beiden Steinen nach rechts und unten. Der linkeste Stein (mostLeftStone) liegt somit im Subtract und die erste SteinPos wird daher als 2. Tupel übergeben, da wir sonst einen Stein zurück bekommen der bereits im ursprünglichen Set enthalten ist
                ...
                returnDiagonal(-1, 1, StoneSet(0), StoneSet(StoneSet.size-1)) // wir gehen in den ersten beiden Steinen nach links unten. Der linkeste Stein findet sich per Subtract über das letzte StonePosTupel
    ...
    def returnDiagonal(xChange: Int, yChange: Int, StoneSetAdd: (Int, Int), StoneSetSubtract : (Int, Int)) : List[(Int, Int)] = {
